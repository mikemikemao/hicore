...
*/
void 
_eXosip_define_get_ip_1(char *locip, int size, const char *compareIp)
{
	if (!locip || (size <= 0) || !compareIp)
	{
		return;
	}
	int type = 0;
								 ...
								 const char *from, const char *proxy)
{
	...
	*dest = NULL;
	...
	if (from)
	{
		snprintf(compareIp, sizeof(compareIp), "%s", strstr(from, "@") + 1);
		
		ip_len = (int) strspn(compareIp, "0123456789.");
		if(ip_len > 0)
		{
			ip_len ++;
			if(ip_len > sizeof(compareIp))
			{
				ip_len = sizeof(compareIp);
			}
			
			snprintf(comapreTmpIp, sizeof(comapreTmpIp), "%s", compareIp);
			snprintf(compareIp, ip_len, "%s", comapreTmpIp);
		}
		
		_eXosip_define_get_ip_1(locip, sizeof(locip), compareIp);
	}
	else if (to)
	{
		snprintf(compareIp, sizeof(compareIp), "%s", strstr(to, "@") + 1);
