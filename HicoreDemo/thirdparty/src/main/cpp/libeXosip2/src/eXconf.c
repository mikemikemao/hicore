...
/*
	type: 0-only eth0 interface; 1-both eth0 & wlan0 interface
*/
void _get_local_ip_default(int *type, char *eth0Ip, char *wlan0Ip, char *wlan0Mask)
{
...
typedef struct
{
    P_getLocalIpAddress_F getlocalip_callback;
}SYSTEM_CALLBACK_T, *PSYSTEM_CALLBACK_T;

SYSTEM_CALLBACK_T g_system_callback_func =
{   
    .getlocalip_callback = _get_local_ip_default,
};


void set_get_localip_func(P_getLocalIpAddress_F p)
{
	g_system_callback_func.getlocalip_callback = p;
}

P_getLocalIpAddress_F get_get_localip_func()
{
	return g_system_callback_func.getlocalip_callback;
}
#endif
